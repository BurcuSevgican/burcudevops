...
- At `General Tab`, select discard old build  `Strategy` and for `Days to keep builds` enter `5` and `Max # of builds to keep` enter `1`.
...
- Create a java file on the `jenkinsfile-pipeline-project` local repository(we have created in *** Hands-on-02 Part 2*** ), name it as `Hello.java`, add coding to print `Hello from Java` and save.
